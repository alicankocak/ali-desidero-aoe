...
export default function PlayerClient({ initial }: { initial: ApiData }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/auth/session")
      .then((r) => r.json())
      .then((s) => setCanEdit(!!s?.user?.email && s.user.email === initial.player.user?.email));
    ...
    <div className="space-y-3">
      ...
      <Card>
        <CardContent className="flex flex-col gap-4 p-4 sm:flex-row sm:items-center">
          ...
          <div className="space-y-1">
            ...
            <div className="text-sm text-gray-600">{p.user?.name || "-"} • {p.user?.email || "-"}</div>
      ...
      <Tabs defaultValue="overview" className="mt-2">
                ...
                {last5.map((m) => (
                  <li key={m.id} className="rounded-xl border p-2 text-sm">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs text-gray-600">{m.score}</span>
                    </div>
                    <div className="mt-1">
            ...
            <CardContent className="space-y-3 p-4">
              ...
              <ul className="grid gap-2 sm:grid-cols-2">
                {videos.map((v) => {
                  ...
                  return (
                    <li key={v.id} className="rounded-xl border p-3 space-y-2">
        ...
        <TabsContent value="message">
          <Card><CardContent className="p-4 text-sm text-gray-600">Özel mesaj kutusu burada olacak.</CardContent></Card>
        </TabsContent>
      </Tabs>
    </div>
  );
